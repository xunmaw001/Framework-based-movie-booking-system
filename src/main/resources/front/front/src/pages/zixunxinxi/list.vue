...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 10px 0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
	    <div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>资讯标题</div>
        <el-input v-model="formSearch.zixunbiaoti" placeholder="资讯标题" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}'>
		<div class="lable" v-if="false" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>资讯类型</div>
        <el-select v-model="formSearch.zixunleixing" placeholder="请选择资讯类型" :clearable="true">
          <el-option v-for="(item, index) in zixunleixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('zixunxinxi','新增')" :style='{"cursor":"pointer","padding":"0px 10px","margin":"0 4px 0 0","borderColor":"#fda100","color":"#fda100","outline":"none","borderRadius":"4px","background":"#fcf2e1","borderWidth":"1px 4px","width":"auto","fontSize":"14px","lineHeight":"42px","borderStyle":"solid","height":"42px"}' type="primary" @click="add('/index/zixunxinxiAdd')"><i v-if="true" :style='{"color":"#fda100","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"margin":"20px 0 10px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0 10px","margin":"30px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
			<div :style='{"padding":"10px","boxShadow":"0px 2px 6px #e6e6e6","margin":"0 0 20px","borderRadius":"8px","flexWrap":"wrap","background":"#fff","display":"flex","width":"23.5%","position":"relative","justifyContent":"space-between","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				...
				<div :style='{"width":"100%","padding":"4px 10px","lineHeight":"24px","fontSize":"16px","color":"#249940","textAlign":"center"}' class="name ">资讯标题:{{item.zixunbiaoti}}</div>
				<div :style='{"width":"100%","padding":"4px 10px","lineHeight":"24px","fontSize":"16px","color":"#249940","textAlign":"center"}' class="name ">资讯类型:{{item.zixunleixing}}</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/zixunleixing/zixunleixing').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.curFenlei != '全部') searchWhere.zixunleixing = this.curFenlei;
        this.$http.get('zixunxinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/zixunxinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
